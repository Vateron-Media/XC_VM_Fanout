...
func AnnexB(dst []byte, s Sample, t *Track) ([]byte, error) {
	return AnnexBFromAVCC(dst, s.Data, t, s.Sync)
}

// AnnexBFromAVCC is AnnexB over a sample's raw AVCC bytes, for a caller that has
// the data and its random-access flag but not a whole Sample (the interleaved
// remux merges samples of both tracks and carries only what it needs).
func AnnexBFromAVCC(dst, data []byte, t *Track, sync bool) ([]byte, error) {
	...
	for off := 0; off < len(data); {
		if off+t.NALULengthSize > len(data) {
		...
		dst = append(append(dst, startCode...), data[off:off+n]...)
		off += n
	}
	return dst, nil
}
...
// understands.
//
// The whole and fractional parts are scaled separately so the multiply cannot
// overflow. A media segment's tfdt is the source's ABSOLUTE decode time and
// grows without bound on a live stream, and a high-timescale source (10 MHz is
// not unusual, and the spec allows far more) times t*90000 out of int64 in
// months — or, at a GHz timescale, in a day — which would turn every timestamp
// past that point into non-monotonic garbage. The split keeps it exact: t/ts is
// whole seconds' worth, and t%ts is under one timescale so its product stays
// small whatever the timescale is.
func Scale(t int64, timescale uint32) int64 {
	if timescale == 0 {
		return 0
	}
	ts := int64(timescale)
	return (t/ts)*90000 + (t%ts)*90000/ts
}
